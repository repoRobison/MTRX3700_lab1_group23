...
// must match piano_tiles.qsf EXACTLY, brackets included -- Quartus silently
...
//
//   lane_fsm (in lane_fsm.sv) is the other answer. It is deliberately NOT
//   instantiated. Keep the file until the group ratifies this choice; swapping
//   back means replacing note_scheduler + lane_controller + hit_detector with
//   four lane_fsm instances and a spawn generator, and rewriting the README's
//   Task 2 / Task 3 boundary.
//
// R0 (reuse all Reaction Time Game modules) is satisfied through the hierarchy
// rather than by instantiating each one here:
//     synchroniser -> inside debounce
//     debounce     -> inside button_input   (7 instances: 4 keys, 3 switches)
//     timer        -> inside beat_gen       (2 instances: beat and phase)
//     rng          -> instantiated below, unmodified
//     seven_seg    -> instantiated below x6, unmodified
//
...
module top_level #(
    ...
    // button_input guarantees that level is settled before its power-on reset
    // releases. During play, remember the prior debounced value and restart the
    // whole game for exactly one clock when it changes.
    always_ff @(posedge CLOCK_50) begin
        level_q <= level;
    end

    assign level_change_reset = !reset && (level != level_q);
    assign game_reset         = reset | level_change_reset;
    // Reaching 99 stops every gameplay subsystem, but deliberately does not
    // reset the score. SW0 or a level change clears the terminal score.
    assign round_reset        = game_reset | won;
    ...
    // R9's terminal state: score holds at 99, round_reset clears all live notes,
    // and the LED override below flashes until SW0 or a level change resets.

    localparam int WIN_DIV_W   = $clog2(CLKS_PER_MS);
    localparam int WIN_PHASE_W = $clog2(`GP_WIN_FLASH_MS + 1);
    logic [WIN_DIV_W-1:0]   win_ms_div;
    logic [WIN_PHASE_W-1:0] win_phase_ms;
    logic                   win_led_on;

    always_ff @(posedge CLOCK_50) begin
        if (game_reset || !won) begin
            win_ms_div   <= '0;
            win_phase_ms <= '0;
            win_led_on   <= 1'b1;
        end else if (win_ms_div == WIN_DIV_W'(CLKS_PER_MS - 1)) begin
            win_ms_div <= '0;
            if (win_phase_ms == WIN_PHASE_W'(`GP_WIN_FLASH_MS - 1)) begin
                win_phase_ms <= '0;
                win_led_on   <= ~win_led_on;
            end else begin
                win_phase_ms <= win_phase_ms + WIN_PHASE_W'(1);
            end
        end else begin
            win_ms_div <= win_ms_div + WIN_DIV_W'(1);
        end
    end

    logic [3:0] score_tens, score_units;

    score_bcd u_score_bcd (
        .value (score_value),
        .tens  (score_tens),
        .units (score_units)
    );
    ...
    // Displays
    //=========================================================================
    // R5 costs no extra logic: seven_seg's existing default arm drives all
    // segments off for any bcd greater than 9, so an inactive lane just stops
    // driving a real digit. This is why GP_BLANK_DIGIT is 4'hF and why the
    // reused decoder is used unmodified rather than given an `active` input.
    logic [3:0] digit0, digit1, digit2, digit3;

    assign digit0 = (!won && lane_active[0]) ? disp_c0 : `GP_BLANK_DIGIT;
    assign digit1 = (!won && lane_active[1]) ? disp_c1 : `GP_BLANK_DIGIT;
    assign digit2 = (!won && lane_active[2]) ? disp_c2 : `GP_BLANK_DIGIT;
    assign digit3 = (!won && lane_active[3]) ? disp_c3 : `GP_BLANK_DIGIT;

    seven_seg u_hex0 (.bcd(digit0),      .segments(HEX0));
    seven_seg u_hex1 (.bcd(digit1),      .segments(HEX1));
    seven_seg u_hex2 (.bcd(digit2),      .segments(HEX2));
    seven_seg u_hex3 (.bcd(digit3),      .segments(HEX3));

    // Brief 2.1: "HEX5 - HEX4 Two digit score (00-99)". Tens on HEX5.
    seven_seg u_hex4 (.bcd(score_units), .segments(HEX4));
    seven_seg u_hex5 (.bcd(score_tens),  .segments(HEX5));
    ...
    assign LEDR = won ? {10{win_led_on}} : gameplay_leds;
